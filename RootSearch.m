...
Options[RootSearch]={InitialSamples\[Rule]300,
      RootTest\[RuleDelayed]
        (Abs[#2]\[LessEqual]10^5(Abs[#1]+$MinMachineNumber)$MachineEpsilon&),
      MaxBrentSteps\[Rule]150,MaxSecantSteps\[Rule]150,
...
RootSearch[
      lhs_\[Equal]rhs_,{x_Symbol?(!NumericQ[#]&),xmin1_?NumericQ,
        xmax1_?NumericQ},opts___?OptionQ]/;
    Im[N[xmin1,17]]\[Equal]0&&Im[N[xmax1,17]]\[Equal]0&&Re[xmin1]<Re[xmax1]:=
  Module[{f,u,invalidQ,InitSmpls,Test,MaxBrntStps,MaxSecntStps,PrecGoal,
    ...
    If[PrecGoal\[Equal]InitPrec\[Equal]$MachinePrecision,
      GrdDigts=0;{xmin,xmax}\[Equal]N[{xmin,xmax}],(*else*)GrdDigts=
        10;{xmin,xmax}=N[{xmin,xmax},InitPrec]];
    If[opts=!=Null,
      BadOpts=Complement[Part[Flatten[{opts}],All,1],
          Part[Options[RootSearch],All,1],SameTest\[Rule]Automatic];
      If[Length[BadOpts]===1,
    ...
    SymbolList=
      Union@Cases[{lhs,
              rhs}/.{_Rule\[Rule]Sequence[],_RuleDelayed\[Rule]
                Sequence[]},_Symbol?(!NumericQ[#]&&
                  Context[#]=!="System`"&),{-1},Heads\[Rule]False];
    ...
    If[invalidQ,$Failed,
      (*else*)
      f=Evaluate[{SamplePoints[#],lhs-rhs/.x\[Rule]#}]&;
      f=ReplacePart[f,Set,{1,0}];
      u=Function[
          Evaluate[
            withTemp[{setTemp[y,(lhs-rhs)/.x\[Rule]#],
                setTemp[yyp,((lhs-rhs)/D[lhs-rhs,x])/.x\[Rule]#]},
              Hold[SamplePoints[#]=y,
                Which[TrueQ[y\[Equal]0],0,NumericQ[yyp],yyp,True,
                  Indeterminate]]]]];
      u=ReplacePart[
          u/.{withTemp\[Rule]With,setTemp\[Rule]Set},{CompoundExpression},{{1,
              2,0}},{{1}}];
      If[TrueQ@FreeQ[u,Derivative,Heads\[Rule]True],
        roots=
      ...
      If[roots==={},{},
        ...
        If[PrecGoal\[NotEqual]$MachinePrecision,
          (*Bring Precision back down.*)roots=
            Map[If[TrueQ[(#\[Equal]0)&&(Accuracy[#]>PrecGoal)],
                  SetAccuracy[#,PrecGoal],#]&,N[roots,PrecGoal],
              Heads\[Rule]False],(*else*)roots=N[roots]
          ];
        Map[{x\[Rule]#}&,roots,Heads\[Rule]False]
        ]
      ]
    ]
...
HoldPattern[
    RootSearch[_,
        arg2:{_,xmin_,xmax_},___]/;(Im[N[xmin,17]]\[NotEqual]0||
            Im[N[xmax,17]]\[NotEqual]0||Re[xmax]\[LessEqual]Re[xmin]||
...
SearchForRootsU=
    Function[{f,u,xmin,xmax,MaxSecantIter,MaxBrentIter,InitialSamples,
      ...
      Module[{xList,yList,ZeroPosn,CrossingPosn1,CrossingPosn2,CrossingVals1,
        ...
        yList=
          Developer`ToPackedArray[MakeReal[Map[f,xList,Heads\[Rule]False]]];
        If[FreeQ[yList,_?NumericQ,{1}],
          Message[RootSearch::numb];{},
          (*else we move on.*)
          ZeroPosn=Position[yList,_?(#\[Equal]0&),Heads\[Rule]False];
          If[PrecGoal\[Equal]$MachinePrecision,
            soln1=Extract[xList,ZeroPosn],
            (*else*)
            soln1=SetPrecision2[Extract[xList,ZeroPosn],PrecGoal+GrdDigts];
            zeroList=MakeReal[Map[f,soln1,Heads\[Rule]False]];
            ZeroPosn=Position[zeroList,_?(#\[Equal]0&),Heads\[Rule]False];
          ...
          If[CrossingPosn2=!={},
            ...
            CrossingVals2=
              Map[Take[xList,#]&,CrossingPosn2,Heads\[Rule]False];
            ...
            soln2=Apply[zBrent[u,##,MaxBrentIter,InitPrec,PrecGoal+GrdDigts]&,
                CrossingVals2,1,Heads\[Rule]False],
            (*else*)soln2={}
            ];
          ...
          CrossingPosn1=
            Complement[CrossingPosn1,CrossingPosn2,SameTest\[Rule]Automatic];
          If[CrossingPosn1=!={},
            
            CrossingVals1=
              Map[Take[xList,#]&,CrossingPosn1,Heads\[Rule]False];
            
            If[PrecGoal\[NotEqual]$MachinePrecision,
              CrossingVals1=SetPrecision2[CrossingVals1,PrecGoal+GrdDigts]];
            
            soln3=Apply[zBrent[f,##,MaxBrentIter,InitPrec,PrecGoal+GrdDigts]&,
                CrossingVals1,1,Heads\[Rule]False],
            (*else*)soln3={}
            ];
          ...
          (*Prepare to Apply zBrent on u[
                ...
                x] changes sign.*)
          If[MinPos1=!={},
            xs=Apply[Take[xList,{#-1,#+1}]&,MinPos1,{1},Heads\[Rule]False];
            signs=Map[Sign2[MakeReal[u[#]]]&,xs,{-1},Heads\[Rule]False];
            ...
            MinPos2=Extract[MinPos1,
                Position[
                  Replace[
                    signs,{{1,1,-1}\[Rule]0,{-1,1,1}\[Rule]0,{-1,-1,
                          1}\[Rule]0,{1,-1,-1}\[Rule]0},{-2}],_List,1]];
            
            xMinVals=
              Transpose[{xs,
                  Replace[
                    signs,{{1,1,-1}\[Rule]{2,3},{-1,1,1}\[Rule]{1,2},{-1,-1,
                          1}\[Rule]{2,3},{1,-1,-1}\[Rule]{1,
                          2},_\[Rule]0},{-2}]}];
          ...
          If[MinPos2=!={},
            ...
            If[PrecGoal\[NotEqual]$MachinePrecision,
              ...
              yPnts=Map[f,xPnts,{-1},Heads\[Rule]False],
              (*else*) 
              yPnts=Apply[Take[yList,{#-1,#+1}]&,MinPos2,{1},
                  Heads\[Rule]False]
              ];
            
            xyPnts=Replace[Transpose[{xPnts,yPnts}],List\[Rule]Sequence,{3},
                Heads\[Rule]True];
            
            soln5=Apply[
                GoldenSecant[f,##,MaxSecantIter,MaxBrentIter,InitPrec,
                    PrecGoal+GrdDigts]&,xyPnts,1,Heads\[Rule]False],
            (*else*)soln5={}
            ];
    ...
    Function[{f,xmin,xmax,MaxSecantIter,MaxBrentIter,InitialSamples,InitPrec,
      ...
      Module[{xList,yList,ZeroPosn,CrossingPosn1,CrossingVals1,MinPos1,soln1,
        ...
        If[FreeQ[yList,_?NumericQ,{1}],Message[RootSearch::numb];{},
          ...
          CrossingPosn1=
            Complement[CrossingPosn1,ZeroPosn,SameTest\[Rule]Automatic];
          CrossingPosn1=Transpose[{CrossingPosn1,CrossingPosn1+1}];
          (*Find samples near a local min of Abs[f[x]].*)
          ...
          (*Prepare to Apply GoldenSecant on f[x] where Abs[
                f[x]] has a local min.*)
          If[MinPos1=!={},
            xPnts=Map[Take[xList,{#-1,#+1}]&,MinPos1,Heads\[Rule]False];
            If[PrecGoal\[NotEqual]$MachinePrecision,
              xPnts=SetPrecision2[xPnts,PrecGoal+GrdDigts];
              yPnts=MakeReal[Map[f[#]&,xPnts,{-1},Heads\[Rule]False]],
              (*else*) 
              yPnts=Map[Take[yList,{#-1,#+1}]&,MinPos1,Heads\[Rule]False]
              ];
...
aproblem with roundoff error. 
        ...
        If[dx>100*xw,
          ...
          If[++GiantStepCount\[Equal]4,(*Assume we are not converging on a \
root.*)Throw[False],(*else*)dx=xw(2-GoldenRatio)],
          (*else*)
          GiantStepCount=0;
          If[NonNegativeQ[dx-xw],
            dx=xw(2-GoldenRatio),
            (*else*)If[direction\[NotEqual]PrevDirection,
        ...
        If[FuncPrec\[Equal]$MachinePrecision,
          x2+dx*direction,
          (*else*)Block[{$MinPrecision=-Infinity},
...
SetPrecision2=
    Function[{expr,prec},
      Replace[expr,{x_?(#\[Equal]0&)\[RuleDelayed]SetAccuracy[x,prec],
          x_?NumericQ\[RuleDelayed]SetPrecision[x,prec]},{-1}]];

If[FreeQ[Options[Precision],Verbatim[Rule][Round,_]],
    Precision2=(Block[{$MinPrecision=-\[Infinity]},Precision[#]]&),
    Precision2=(Block[{$MinPrecision=-\[Infinity]},
            Precision[#,Round\[Rule]False]]&)
    ];
...
Sign2[x_?InexactNumberQ]:=
    If[Precision[x]\[GreaterEqual]3.0,Sign[x],Sign@SetPrecision[x,15.0]];
Sign2[DirectedInfinity[z_]]:=Sign2[z];
Sign2[x_]:=Sign[x]



LocalExtremaQ=((Part[#,2]\[NotEqual]0)&&PositiveQ[First[#]/Part[#,2]-1]&&
...
CrossingPosition=Function[{xList,PosnList,u},
      Module[{lst,CrossingVals},
        CrossingVals=Map[Take[xList,#]&,PosnList,Heads\[Rule]False];
        lst=
          Thread[{Apply[{MakeReal@u[#1],MakeReal@u[#2]}&,CrossingVals,1,
                Heads\[Rule]False],PosnList}];
...
Ulp2=Function[{x1,x2},
      Module[{eps,sgn=Sign2[x2-x1],tol},
        Which[
          ...
          While[(eps=Plus@@{x1+sgn*tol,-x1})\[Equal]0.0,tol*=2];
          Abs[eps],
          (*test3*)True,
          Block[{$MinPrecision=-Infinity},
            tol=SetPrecision[16*10^-Accuracy[x1],20];
            
            While[(eps=Plus@@{x1+sgn*tol,-x1};Precision[eps]\[LessEqual]0),
              tol*=2];
...
InitialPoints=
    Function[{xmin,xmax,n,InitPrec,PrecGoal},
      If[InitPrec\[Equal]$MachinePrecision,
        Module[{step=Re@N[xmax-xmin]/(n-1),pnts},
          ...
          With[{noise=step/50,state=$RandomState,xmin2=Re@N@xmin,
            ...
            SeedRandom[state];
            Union[pnts,SameTest\[Rule]Automatic]
            ]],
        ...
        Module[{step=Re@SetPrecision[(xmax-xmin),InitPrec]/(n-1),pnts,eps},
          With[{
              ...
              xmin2=If[xmin\[NotEqual]0,Re@SetPrecision2[xmin,InitPrec],
                  Re@SetAccuracy[xmin,PrecGoal]],
              xmax2=If[xmax\[NotEqual]0,Re@SetPrecision2[xmax,InitPrec],
            ...
            SeedRandom[state];
            Union[pnts,SameTest\[Rule](#1-#2\[Equal]0&)]
            ]]
        ]];
      ...
      Module[{a,b,c,fa,fb,fc,d,e,p,q,r,s,tol1,xm,newB,newF,iter,biggerSample,
        ...
        ({a,b,c,biggerSample}={x1,x2,x2,0};
          ...
          Passed=Catch[Do[
                ...
                {b,fb}={newB,newF};
                If[fb\[Equal]0,(*Found an exact root*)Throw[True]],
                {maxIter}]];
          If[TrueQ[Passed],b,{}]
          )]];
